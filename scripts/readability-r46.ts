  ...
  for (let i = 0; i < parts.length; i++) {
    if (!parts[i].startsWith('<')) {
      ...
      if (sentences) {
        ...
        // if no sentences match, it might just be whitespace or something small
        parts[i] = parts[i];
      }
    }
  }
...
async function main() {
  const slugs = [
    "fix-pantum-app-not-printing-wifi-setup-offline-windows",
    "polaroid-hi-print-first-time-setup-cartridge-loading-cleaning-guide",
    "hp-printer-grinding-noise-when-printing",
    "instant-ink-cartridge-shows-empty-but-full",
