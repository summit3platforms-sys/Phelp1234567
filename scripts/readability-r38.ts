...
async function main() {
    const slugs = [
        "instax-mini-link-blank-overexposed-dark-prints-fix",
        "star-micronics-cutter-locked-paper-jam-cover-wont-open",
        "fix-pantum-output-bin-errors-wont-turn-on-factory-reset",
        "hp-envy-6455e-setup-problems",
        "fix-citizen-faded-print-streaky-lines-ribbon-tension"
    ];
    ...
    const updates = articles.map(article => {
        const originalWords = article.content.split(/\s+/).length;
        const newContent = rewrite(article.content);
        const newWords = newContent.split(/\s+/).length;
        console.log(`Rewrote ${article.slug}: ${originalWords} -> ${newWords} words`);
    ...
    console.log("Successfully updated all 5 articles.");
}
